<?php
	...
	*/
	class HttpRequest
	{
		...
		/**
		 * Parses multipart/form-data into respective RESTful superglobal.
		 * PATCH body data would go into the $_PATCH array
		 */
			...
			while (isset($rawBody[$index])) {
				...
				if ($state === "PARSE_BOUNDARY") {
					...
					if ($buffer === "--" . $boundary) {
						...
						++$index;
//						if ($char === "\r" && $nextChar === "\n"){
//							// Line over, but no boundary parsed
//							$buffer = "";
//							$state = "PARSING_PACKET";
//							$index += 2;
//						}else{
//							// Consume
//							$buffer .= $char;
